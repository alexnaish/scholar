...
describe('Candidate API ', function () {
	...
	it('/api/candidate/:name/:id/raw should handle invalid ids', function (done) {
	...
	it('/api/candidate/:name/:id/raw should 404 if no such candidate', function (done) {

		request.get('/api/candidate/letsMakeUpAName/123123123123123123123123/raw')
			.expect(404)
			.end(function (err, res) {
				expect(err).to.equal(null);
				done();
	...
	it('/api/candidate/:name/:id/raw should return a standard server error if something goes wrong', function (done) {
		var CandidateService = require('./service');
		var errorStub = sinon.stub(CandidateService, 'findOne').yields({
			message: 'some error'
		}, null);
		...
		request.get('/api/candidate/letsMakeUpAName/123123123123123123123123/raw')
			...
			.end(function (err, res) {
				...
				CandidateService.findOne.restore();
				done();
			});
	});
	...
	it('/api/candidate should return a 500 status and an error message if an error occurs', function (done) {
		...
		var CandidateService = require('./service');
		var errorStub = sinon.stub(CandidateService, 'findDistinct').yields({
