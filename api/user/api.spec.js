...
describe('Users API', function() {
    ...
    describe('POST', function() {
        ...
        it('/api/user should return 201 and create a new user', function(done) {
            ...
            request.post('/api/user')
                .set('Authorization', 'Bearer ' + generatedToken)
                .send({
                    credentials: helpers.generateCredentialsToken(validUser)
                })
                .expect('Content-Type', /json/)
                .expect(201)
                .end(function(err, res) {
        ...
        it('/api/user should return 400 if payload is missing credentials', function(done) {
            request.post('/api/user')
                .set('Authorization', 'Bearer ' + generatedToken)
                .send({})
                .expect('Content-Type', /json/)
                ...
                .end(function(err, res) {
                    expect(err).to.equal(null);
                    expect(res).to.not.equal(null);
                    expect(res.body).to.have.property('error', 'Invalid credentials token');
                    done();
                });
        });

        it('/api/user should return 400 if payload is missing key fields', function(done) {
            request.post('/api/user')
                .set('Authorization', 'Bearer ' + generatedToken)
                .send({
                    credentials: helpers.generateCredentialsToken({
                        username: 'blah123',
                        firstName: 'abc'
                    })
                })
        ...
        it('/api/user should return 409 if user email already taken', function(done) {
            ...
            request.post('/api/user')
                .set('Authorization', 'Bearer ' + generatedToken)
                .send({
                    credentials: helpers.generateCredentialsToken(specificUser)
                })
                .expect('Content-Type', /json/)
                .expect(409)
                .end(function(err, res) {
    ...
    describe('GENERATE TOKEN', function() {
        ...
        it('/user/token should be able to handle missing credentials', function(done) {
            request.post('/api/user/token')
                .expect('Content-Type', /json/)
                .expect(400)
                .end(function(err, res) {
        ...
        it('/user/token should be able to handle invalid credential tokens', function(done) {
            request.post('/api/user/token')
                .send({
                    credentials: 'this.is.not.valid'
                })
                .expect('Content-Type', /json/)
                .expect(400)
                .end(function(err, res) {
        ...
        it('/user/token should return a standard 500 if an error occurs', function(done) {
            sinon.stub(UserService, 'checkLogin').yields({
                message: 'some error'
            }, null);
